...
class EmailClassifier:
    ...
    def keyword_based_classify(self, subject: str, body: str, from_email: str = "") -> str:
        ...
        # 소셜 미디어 알림 체크 (LinkedIn, Facebook, Instagram 등)
        ...
        if is_from_social and has_notification:
            ad_score += 5  # 소셜 미디어 알림은 강력하게 광고로 분류
...
class SettingsDialog:
    ...
    def toggle_custom_settings(self):
        if self.mail_server_var.get() == "사용자 정의":
            self.custom_frame.pack(fill="x", pady=(0, 10), before=self.custom_frame.master.winfo_children()[-2])
...
class TodoApp:
    ...
    def create_widgets(self):
        ...
        # Login Frame
        login_frame = ttk.LabelFrame(self.root, text="IMAP 로그인", padding=10, borderwidth=0, relief="flat")
        login_frame.pack(fill="x", padx=10, pady=5)
        
        # Modernized 로그인 카드형 중앙 배치
        self.login_card = ttk.Frame(self.root, style='Card.TFrame')
        self.login_card.place(relx=0.5, rely=0.18, anchor='n')
        self.username_entry = ttk.Entry(self.login_card, textvariable=self.username_var, width=28, foreground="gray")
        self.username_entry.grid(row=0, column=0, padx=32, pady=(32, 12), sticky='ew')
        ...
        self.password_entry.grid(row=1, column=0, padx=32, pady=(0, 12), sticky='ew')
        ...
        ttk.Button(self.login_card, text="⚙ 환경설정", command=self.open_settings).grid(row=2, column=0, padx=32, pady=(0, 12), sticky='ew')
        self.login_card.columnconfigure(0, weight=1)

        # 이메일 불러오기 버튼 (로그인 카드 아래)
        self.fetch_button = ttk.Button(self.root, text="이메일 불러오기", command=self.fetch_emails_handler)
        self.fetch_button.place(relx=0.5, rely=0.36, anchor='n')
        ...
        self.todo_tree.column("마감일", width=120)
        ...
        self.tree.column("마감일", width=120)
        self.tree.column("제목", width=300)
        self.tree.column("발신자", width=150)
        self.tree.column("날짜", width=120)
        ...
        # Category Frame
        category_frame = ttk.LabelFrame(detail_category_frame, text="분류 관리", padding=10, borderwidth=0, relief="flat")
    ...
    def on_todo_select(self, event):
        """할일 목록 선택 이벤트"""
        # 선택된 항목이 있으면 전체 메일 탭에서도 해당 메일 선택
        ...
        if 0 <= idx < len(self.emails_data):
            # 전체 메일 탭에서 해당 메일 찾기 (나중에 뷰 전환 시 사용)
            pass
